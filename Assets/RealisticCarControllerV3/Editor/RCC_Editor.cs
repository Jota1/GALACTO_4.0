...
public class RCC_Editor : Editor {
	...
	public override void OnInspectorGUI () {
		...
		if(WheelSettings){
			...
			if(GUILayout.Button("Create Necessary Gameobject Groups")){
				...
				if(!didWeHaveThisObject){
					...
					EditorUtility.DisplayDialog("Vehicle has these groups already!", "Vehicle has these groups already!", "Close");
			...
			if(GUILayout.Button("Create Wheel Colliders")){
				...
				if(wheelColliders.Length >= 1)
					EditorUtility.DisplayDialog("Vehicle has Wheel Colliders already!", "Vehicle has Wheel Colliders already!", "Close");
				else
		...
		if(SteerSettings){
			...
			EditorGUILayout.PropertyField (serializedObject.FindProperty ("steeringSensitivity"), new GUIContent ("Steering Sensitivity"), false);
			EditorGUILayout.Space();
...
//			EditorGUILayout.PropertyField(serializedObject.FindProperty("autoReverse"), new GUIContent("Auto Reverse", "Enables/Disables auto reversing when player press brake button. Useful for if you are making parking style game."));
			...
			if(carScript.wheelTypeChoise == RCC_CarControllerV3.WheelType.BIASED){
