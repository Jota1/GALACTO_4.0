...
//			}
		...
		if(foldControllerSettings){
			...
			controllerTypeStrings.Add("Keyboard");	controllerTypeStrings.Add("Mobile");	controllerTypeStrings.Add("XBox");	controllerTypeStrings.Add("Custom");
			EditorGUILayout.BeginVertical (GUI.skin.box);
		...
		if(RCCSettingsAsset.controllerSelectedIndex == 1){
			...
			GUI.color = new Color(.75f, 1f, .75f);
			EditorGUILayout.HelpBox("All UI buttons will feed the vehicles at runtime.", MessageType.Info);
			GUI.color = originalGUIColor;
		...
		if(RCCSettingsAsset.controllerSelectedIndex == 2){
				...
				EditorGUILayout.PropertyField(serializedObject.FindProperty("Xbox_boostKB"), new GUIContent("Boost"));
